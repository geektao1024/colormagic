<template>
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
    ...
    <div class="lg:col-span-5 bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-700">
      ...
      <p class="text-lg mb-6 text-gray-300">{{ $t('imageColorPicker.seoDescription') }}</p>
      ...
      >
        ...
        >
          <div class="border border-gray-700 rounded-lg p-4 bg-gray-850 mb-4">
            <img
              v-if="state.dataUrl !== ''"
              :src="state.dataUrl"
              class="w-full h-48 object-contain bg-gray-750 rounded mb-4 shadow-sm"
            >
            <UInput
              type="file"
              size="xl"
    ...
    <div class="lg:col-span-7 bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-700">
      ...
      <!-- 空状态 -->
      <div v-else-if="!extractedColors.length" class="flex flex-col items-center justify-center py-12 border border-dashed border-gray-700 rounded-lg bg-gray-850">
        <UIcon name="i-heroicons-photo" class="text-gray-400 h-12 w-12 mb-4" />
        <p class="text-gray-400">{{ $t('app.imageColorPicker.emptyState') }}</p>
      </div>
