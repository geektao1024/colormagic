...
const title = `Professional Color Palettes - AI-Generated by ColorMagic | Designer-Grade`;
const description = `Explore ${count.value}+ designer-grade color palettes created with AI color theory. Find perfect harmonious color schemes for brand design, UI/UX, and creative projects.`;
...
const jsonLd = {
  ...
  'url': `${siteUrl}/palette/explore`,
  'mainEntity': {
    '@type': 'ItemList',
    'itemListElement': palettes.value.slice(0, 10).map((palette, index) => ({
      '@type': 'ListItem',
      'position': index + 1,
      'item': {
        '@type': 'Thing',
        'name': palette.text,
        'url': `${siteUrl}/palette/${palette.id}`
      }
    }))
  }
};
...
useSeoMeta({
  title,
  description,
  keywords: 'professional color palettes, designer-grade color schemes, AI color palette generator, color harmonies, creative color combinations, brand color system, context-aware palette system',
  ogTitle: title,
  ogDescription: description,
  ogType: 'website',
  ogImage: `${siteUrl}/img/og.png`,
  ogUrl: `${siteUrl}/palette/explore`,
  twitterTitle: title,
  twitterDescription: description,
  twitterCard: 'summary_large_image',
  twitterImage: `${siteUrl}/img/og.png`,
...
useHead({
  link: [
    {
      rel: 'canonical',
      href: `${siteUrl}/palette/explore${locale.value !== 'en' ? '?lang=' + locale.value : ''}`
    }
  ],
  script: [
    {
